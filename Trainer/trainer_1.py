...
if showexample != 1:
    ...
    for i in range(1):
        print("Image:",dataset.__getitem__(showexample)[i].shape,"maxval:",np.max(dataset.__getitem__(showexample)[i].detach().numpy()),"minval:",np.min(dataset.__getitem__(showexample)[i].detach().numpy()))
